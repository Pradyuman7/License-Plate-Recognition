...
def imclearborder(imgBW, radius):
    ...
    contours, hierarchy = cv2.findContours(imgBWcopy.copy(), cv2.RETR_TREE,
    ...
    for idx in contourList:
        cv2.drawContours(imgBWcopy, contours, idx, (255, 0, 0), 2)
...
def bwareaopen(imgBW, areaPixels):
    ...
    for idx in np.arange(len(contours)):
        ...
        if (area >= 0 and area <= areaPixels):
            cv2.drawContours(imgBWcopy, contours, idx, (0, 255, 0), 2)

    return imgBWcopy


def startcode(img):
# img = cv2.imread('/Users/pradyuman.dixit/Desktop/Students_upload/images/car.jpg', 0)
